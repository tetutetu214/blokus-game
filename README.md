...
| LOCAL 2P | ローカル1対1（本家ブロックス2人ルール。各プレイヤーが対角2色＝2トレイを担当し、盤をフルに使う） |
| LOCAL 4P | ローカル対戦（1台で交代プレイ） |

#### LOCAL 2P のルール

ブロックスの公式2人ルールを採用。各プレイヤーは対角の2つの角（2トレイ分のピース）を担当し、**自分のピースは1色＝1つの陣地として一体で扱う**。

- P1 = 青（slot 0,2 / 左上＋右下の角）、P2 = 橙（slot 1,3 / 右上＋左下の角）
- 自分のピース同士は辺で接触NG・角で接触OK（色＝チーム単位でルール適用）。相手とは辺接触OK
- スコアは自分の2トレイの合計。高い方が勝ち（同点はDRAW）

内部の4人エンジンは無改造のまま `teamOf()` で2チームに束ねる方式で実装しており、4人モードの挙動は不変。
