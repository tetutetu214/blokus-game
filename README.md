...
## チュートリアル（Issue #52 対応予定）

### 背景
初めてのユーザーが何をすればいいかわからない問題を解決する。

### 形式
ステップバイステップのインタラクティブチュートリアル。
説明を表示 → 実際にピースを置いてもらう → 次の説明 → 次のピースを置いてもらう、を繰り返す。

### 教える内容

#### ① ゲームの目的
- できるだけ多くのピースを盤面に置く
- 最後に手元に残ったピースのマス数が少ないほど勝ち

#### ② 基本ルール（最重要：コーナー接続）
- 自分の最初のピースはコーナー（隅）からスタート
- 2枚目以降は「自分の同色ピースの角（コーナー）」に接するように置く
- 同色ピースの辺（エッジ）同士は接してはいけない

#### ③ 相手ピースとの関係
- 相手の色のピースとは辺・角どちらで接しても問題なし

#### ④ 実際に動かしてもらうパート
- コーナー接続OKな例を1つ配置
- 辺接続NGな例を1つ体験（置けないことを確認）
- 角だけ接続OKな例を1つ配置
...
### 既存の HOW TO PLAY との関係
チュートリアル実装後、既存の HOW TO PLAY（テキストルール説明モーダル）は削除する。
